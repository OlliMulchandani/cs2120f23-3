...
#reduce play rock paper --reduce instead of eval for some reason
...
#check box_containing_hello'
#check Box.put -- a constructor takes the type argument implicitly
--constructor applied to argument, not evaluating anything, just sitting there, the term is of the desired type
#check (@Box.put) --very important
...
#check a_pair_string_nat    -- type of ("Love",4) is (Prod String Nat)
