...
-- when you see '¬', prove that whatever it follows implies (→) False
example : ¬False := λ f => f -- could also return False.elim f

example (P : Prop) : ¬(P ∧ ¬P) := λ ⟨p, np⟩ => np p -- apply function (not p) to Prop (p), gives false

-- transitivity of propositions:
example (P Q R : Prop) : (P → Q) → (Q → R) → (P → R) := λ pq qr => λ p => qr (pq p)
-- function composition:
example (α β γ : Type) : (α → β) → (β → γ) → (α → γ) := λ ab bc => λ a => bc (ab a)

example (P Q R : Prop) : P ∨ (Q ∧ R) → (P ∨ Q) ∧ (P ∨ R)
| Or.inl p => And.intro (Or.inl p) (Or.inl p) -- also written as ⟨Or.inl p, Or.inl p⟩
| Or.inr ⟨q, r⟩ = And.intro (Or.inr q) (Or.inr r)

-- logic is just a collection of axioms
axiom em : ∀ (P : Prop), P ∨ ¬P
--axiom em' : ∀ (P : Prop), P ∨ ¬P

-- Law of the excluded middle
example (X : Prop) : X ∨ ¬X := em X
