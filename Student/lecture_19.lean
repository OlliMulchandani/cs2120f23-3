...
- ∀ (x : T), P x -- P is a predicate that takes an argument of type T, yields a proposition.
Every single one must be true ∀, for every value of type T there is a proof of P x
...
example : ∃ (n : Nat), n % 2 = 0 := ⟨2, rfl⟩ -- use reflexive property
...
variable
  (Dog : Type)
  (Blue : Dog → Prop) -- Blue is a predicate
  (Iris : Dog)
  (all_dogs_blue : ∀ (d : Dog), Blue d)

-- all dogs are blue
#check ∀ (d : Dog), Blue d -- a formal statement in predicate logic
-- Blue d is a type, the argument determines the return type
...
example : ∃ (d : Dog), Blue d := ⟨Iris, iris_is_blue⟩ -- alternate syntax
...
def ex2 : (∃ (n : Nat), True ∧ is_even n) → (∃ (f : Nat), is_even f)
| ⟨w, ⟨t, e⟩ ⟩ => ⟨w, e⟩
...
#check (∃ (p : Person), ∀ (e : Person), Loves e p) →
       (∀ (e : Person), ∃ (p : Person), Loves e p)

example : (∃ (p : Person), ∀ (e : Person), Loves e p) →
          (∀ (e : Person), ∃ (p : Person), Loves e p)
| Exists.intro p everyone_loves_p => -- P is the person everyone loves
  fun (m : Person) => Exists.intro p (everyone_loves_p m) -- proof that for every person (m), m loves p
-- Brilliant
