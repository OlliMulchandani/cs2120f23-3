...
def empty_to_empty (e : Empty) : Empty := nomatch e --nomatch must be applied to a false value, it's the get-out-of-jail card
...
def empty_to_α (α : Type) : Empty → α    := nomatch e --false implies anything is true
--none of these functions can be called, but they are all still inhabited
