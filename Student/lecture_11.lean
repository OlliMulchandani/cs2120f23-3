...
- If *b* and *c* are expressions, then so are the following:
  ...
  - b ↔ c     -- b if and only if c; b and c are equivalent; (b implies c) and (c implies b)
...
prefix:max "¬" => un_exp unary_op.not --not will always bind tightest
infixr:35 " ∧ " => bin_exp binary_op.and --the number is the binding strength (making our own PEMDAS) 
infixr:30 " ∨ " => bin_exp binary_op.or 
infixr:25 " ⇒ " =>  bin_exp binary_op.imp
infixr:20 " ⇔ " => bin_exp binary_op.iff --the r means right associative
...
| var_exp v, i => i v -- base case
| un_exp op e, i => (eval_un_op op) (eval_expr e i)
| bin_exp op e1 e2, i => (eval_bin_op op) (eval_expr e1 i) (eval_expr e2 i) --will only do the binary operator once the two expressions are reduced to the atomic level
