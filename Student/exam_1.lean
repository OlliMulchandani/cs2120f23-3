...
| (atob,btog) => (fun a => btog (atob a))

#eval funny_transitivity (String.length, (fun (n : Nat) => n%2 == 0)) "Hello!"
...
| mk (b : Bread) (cs : Sum Cheese Spread) --sum type somewhere here
...
def topping : Sum Cheese Spread := Sum.inr jam
#check topping
#check (Sandwich.mk wheat topping)

def jam_sandwich := (Sandwich.mk wheat topping)

#check jam_sandwich -- is a Sandwich
