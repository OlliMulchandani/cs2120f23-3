...
--the rule is that if it is raining, I have an umbrella
/-!
evaluate these statements
It's raining and I have an umbrella TRUE
It's raining and I don't have an umbrella FALSE
It's not raining and I have an umbrella TRUE
It's not raining and I don't have an umbrella TRUE
-/
/-!
...
def glue_funs : {α β γ : Type} → (β → γ) → (α → β) → α → γ --this does the same as the commented out version above, just accepts α, β, and γ in a cleaner manner, as explained in the hint
| _, _, _, g, f, x => g (f x) -- the blank variables are just to catch the type variables, even though they won't be used
...
#eval glue_funs double (glue_funs double double) 5
#eval glue_funs double square 5 -- expected 50
#eval glue_funs square double 5 -- expected 100

--the result is dependent on the order of the functions
