...
def id_nat : Nat → Nat
| n => n --pattern matching rule, n is the name of first argument, and function will return n
...
#eval id_nat 7 --argument is 7, you separate the arguments by spaces
...
def id_poly : (α : Type) → α → α --only thing that is changing is the type
| α, v => v --α unused in method of function, bc its only purpose is in the declaration
...
--list in Java takes a type as an argument
...
#eval @id_poly' Nat 7          -- α = Nat, the @ symbol tells it to ignore the implicit argument
#eval @id_poly' String "Hello!"   -- α = String, told explicitly
#eval @id_poly' Bool true       -- α = Bool, told explicitly
...
| f, a => f (f a) --takes function f and value a, and returns the function applied to the nat returned from the functioned applied to a 
