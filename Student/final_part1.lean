...
There exists a natural number, n, that satisfies the proposition square n 4
(or when n is squared, it's equal to 4)

My proof uses Exists.intro, which takes a value (2) and a proof that the
value satisfies the proposition. In this case, the proof is generated by
using rfl with the square constructor sqr
-/
...
-- example : successor 2 3 := successor.suc 2 3 rfl
-- example : ¬ successor 2 4 := λ nono => nomatch nono
