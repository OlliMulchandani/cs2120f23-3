...
-/
inductive Prod (α : Type) (β : Type) --type builder, give it two values and it gives you a type
| mk (a : α) (b : β)  --mk for "make"; (3, true) is of type Prod Nat Bool
...
#check (@Box.put) -- polymorphic, takes any type α 
#check @Box.put Nat 9 --type is Box Nat
def jack_in_a_box := @Box.put String "Jack!" --@ turns of implicit arguments
...
def get {α : Type}: Box α → α --input of type Box α and returns value of type α
| (Box.put s) => s 

def new_box := Box.put "Jack!"
...
#eval (3, (true, "hi")).2.2     -- postfix notation
...
| inl (a : α) --introduce value a of type α on the left
| inr (b : β) --introduce value b of type β on the right
...
#check b_sum1
--#check @(Sum.inl 1)
--def a_sum2 : Sum Nat Bool := a_sum1.inr true
--#check a_sum2
...
def elim_sum2 : 
  (Sum Nat Bool) → --if I know that one of theses two is true
  (Nat → String) → --and I know that if this is true, this happens
  (Bool → String) → --and that if this one is true, this also happens
  String --I can conclude that this will happen
